...
function Location() {
  ...
  return <output data-testid="location">{location.searchStr}</output>;
}
...
function view(path = '/dashboard/standups') {
  ...
  render(
    <QueryClientProvider client={client}>
      <TestRouter
        routeId="/dashboard/_authenticated/standups"
        initialEntries={[path]}
      >
        <StandupsPage />
        <Location />
...
describe('standup loading transitions', () => {
  it.each(['content', 'empty', 'error'] as const)(
    ...
    async (outcome) => {
      ...
      expect(
        await screen.findByRole('status', { name: 'Loading standups…' }),
      ).toBeInTheDocument();
      expect(
