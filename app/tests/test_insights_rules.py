...
def test_three_consecutive_days_of_the_same_blocker_is_a_run():
    r = find_blocker_runs(
        rows((d(14), "waiting on infra"), (d(15), "still waiting on infra"), (d(16), "waiting on the infra team")),
        min_days=3,
    )
    assert len(r) == 1
    assert r[0]["days"] == 3
...
def test_a_different_blocker_breaks_the_run():
    r = find_blocker_runs(
        rows((d(14), "waiting on infra"), (d(15), "need a design review"), (d(16), "waiting on infra")),
        min_days=3,
    )
    assert r == []
...
def test_a_weekend_does_not_break_a_run():
    ...
    r = find_blocker_runs(
        rows((d(17), "waiting on infra"), (d(18), "waiting on infra"), (d(21), "waiting on infra")),
        min_days=3,
    )
    assert len(r) == 1
    assert r[0]["days"] == 3
...
def test_a_missed_weekday_breaks_the_run():
    ...
    """
    args = rows((d(14), "waiting on infra"), (d(16), "waiting on infra"), (d(17), "waiting on infra"))
...
def test_two_separate_runs_are_both_reported():
    r = find_blocker_runs(
        rows(
            (d(1), "waiting on infra"),
            (d(2), "waiting on infra"),
            (d(3), "waiting on infra"),
            (d(10), "design review"),
            (d(11), "design review"),
            (d(14), "design review"),
        ),
...
def test_rows_out_of_order_are_handled():
    r = find_blocker_runs(
        rows((d(16), "waiting on infra"), (d(14), "waiting on infra"), (d(15), "waiting on infra")),
        min_days=3,
    )
